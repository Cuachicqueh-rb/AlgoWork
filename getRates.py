...
class getRates(Requestbase):
    ...
    def get_apys(self, token_address):
        ...
        if self.chain == 'fantom':
            return [round(depositAPR, 4) * 100, round(stableBorrowAPR, 4) * 100]
        else:
            return [round(depositAPY, 4) * 100, round(stableBorrowAPY, 4) * 100]

    def get_rewardAprs_Geist(self, token_address):
        """ Returns Incentive APR's Geist Finance on Fantom"""
        api = requests.get('https://api.geist.finance/api/getApys')
        reward_aprs = json.loads(api.text)

        aprs = []
        addresses = []
        for i in range(len(reward_aprs['data']['apyDetails'])):
            aprs.append(reward_aprs['data']['apyDetails'][i])
            addresses.append([aprs[i]['tokenAddress'], aprs[i]['apy']])

        gTokens = dict(zip(self.store_file['all_address_names'], addresses[::2]))
        dTokens = addresses[::-2]
        dTokens.reverse()
        dTokens = dict(zip(self.store_file['all_address_names'], dTokens))

        deposit_incentive_APR = gTokens[self.get_token_symbol(token_address)][1] * 100
        borrow_incentive_APR = dTokens[self.get_token_symbol(token_address)][1] * 100
        return [round(deposit_incentive_APR, 2), round(borrow_incentive_APR, 2)]
    ...
    def get_rewardAprs(self, token_address):
        ...
        if self.chain == 'fantom':
            return self.get_rewardAprs_Geist(token_address)
        else:
            reserveData = self.get_reserveData(token_address)
