...
class Pair_Trade_Backtester(object):
    ...
    def current_df(self, start, end=None, principal=100):
        df = self.df
        # aa = df.loc[start:] # for 90 days halflife
        # aa = df.loc['2022-08-20':] # for 50 days halflife
        aa = df.loc[start:].iloc[:,:9]
        # aa = df.loc[start:].copy()

        x = df.columns[1] ## ETH
        y = df.columns[0] ## BTC

        aa['cTicker1'] = aa.iloc[:,2].cumsum().apply(np.exp)
        aa['cTicker2'] = aa.iloc[:,3].cumsum().apply(np.exp)
        aa['cstrategy'] = aa['strategy'].cumsum().apply(np.exp)
        aa['$$return'] = principal * aa['cstrategy'] - principal
        aa['total'] = (aa[y] + abs(aa.hr)*aa[x])
        aa['asset1$$buy'] = principal * (aa[y] / aa.total) * aa.position
        aa['asset2$$buy'] = principal * (aa[x]*abs(aa.hr) / aa.total) * -aa.position
        aa['asset1amount'] = aa['asset1$$buy'] / aa[y]
        aa['asset2amount'] = aa['asset2$$buy'] / aa[x]
        aa['asset1_rebalance'] = aa['asset1amount'] - aa['asset1amount'].shift(1)
        aa['asset2_rebalance'] = aa['asset2amount'] - aa['asset2amount'].shift(1)
        return aa
